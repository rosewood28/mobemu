...
public class GRACE extends Node {
    ...
    /**
     * Size in units of an ant used for bandwidth calculations.
     */
    ...
    /**
     * Maximum size of ant memory in storage units.
     */
    private final int antMemoryCapacity;
    ...
    public GRACE(int id, int nodes, Context context, boolean[] socialNetwork, int antMemorySize, int dataMemorySize, int exchangeHistorySize, long seed, long traceStart, long traceEnd) {
        ...
        // Convert ant memory size to storage units
        this.antMemoryCapacity = antMemorySize * GRACE_MESSAGE_SIZE;
    }
    ...
    protected void onDataExchange(Node encounteredNode, long contactDuration, long currentTime) {
        ...
        for (Message message : graceEncounteredNode.dataMemory) {
            ...
            if (shouldAcceptMessage(message, graceEncounteredNode)) {
    ...
    private boolean insertAnt(GraceHelper.Ant ant, Node encounteredNode, long currentTime) {
        ...
        if (antMemoryCapacity != Integer.MAX_VALUE && antMemory.size() + GRACE_ANT_SIZE >= antMemoryCapacity) {
            antMemory.remove(0);
        }
